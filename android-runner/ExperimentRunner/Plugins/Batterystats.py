...
class Batterystats(Profiler):
    ...
    def __init__(self, config, paths):
        ...
        # FIX
        config_file = self.load_json(
            op.join(self.paths["CONFIG_DIR"], self.paths['ORIGINAL_CONFIG_DIR']))
        ...
        if self.type == 'web':
            self.browsers = [BrowserFactory.get_browser(b)(
                config_file) for b in config_file.get('browsers', ['chrome'])]
    ...
    def start_profiling(self, device, **kwargs):
        ...
        website_name = re.search(
            'pixel3/http00008000(.+?)/firefox', self.paths['OUTPUT_DIR']).group(1) + str(self.iterator)

        systrace_file = r'{}systrace_{}_{}.html'.format(
            self.output_dir, device.id, time.strftime('%Y.%m.%d_%H%M%S'))
        logcat_file = '{}logcat_{}_{}.txt'.format(
            self.output_dir, device.id, time.strftime('%Y.%m.%d_%H%M%S'))
        ...
        results_file_name = '{}.csv'.format(website_name)
        results_file = op.join(
            '/home/chinn501/ownCloud/code/GitHub/GreenLab_CachingPWA/android-runner/examples/batterystats/output/', results_file_name)
        if (self.iterator == 2):
            self.iterator = 0
        self.profile = True
    ...
    def get_data(self, device, app):
        ...
        global sysproc
        sysproc = Popen('{} freq idle -e {} -a {} -t {} -o {}'.format(self.systrace,
                                                                      device.id, app, int(self.duration + 5), systrace_file), shell=True)
# FIX
    ...
    def collect_results(self, device, path=None):
        ...
        batterystats_results = BatterystatsParser.parse_batterystats(
            app, batterystats_file, self.powerprofile)

        # Estimate total consumption
        # charge is given in mAh
        charge = device.shell('dumpsys batterystats | grep "Computed drain:"').split(
            ',')[1].split(':')[1]
        volt = device.shell('dumpsys batterystats | grep "volt="').split(
            'volt=')[1].split()[0]
        ...
        with open(results_file, 'w+') as results:
            ...
            writer.writerow(
                ['Start Time (Seconds),End Time (Seconds),Duration (Seconds),Component,Energy Consumption (Joule),Timestamp Start,Timestamp End'])
    ...
    def aggregate_subject(self):
        ...
        current_row.update(
            self.aggregate_battery_subject(self.output_dir, True))
        subject_rows = list()
    ...
    def aggregate_battery_subject(self, logs_dir, joules):
        def add_row(accum, new):
            row = {k: v + float(new[k]) for k, v in accum.items()
...
# FIX
        ...
        for run_file in [f for f in os.listdir(logs_dir) if os.path.isfile(os.path.join(logs_dir, f))]:
            if ('Joule' in run_file) and joules:
                with open(os.path.join(logs_dir, run_file), 'rb') as run:
                    ...
                    runs.append(
                        {k: v / run_total['count'] for k, v in run_total.items() if k != 'count'})
                runs_total = reduce(
                    lambda x, y: {k: v + y[k] for k, v in x.items()}, runs)
        return OrderedDict(
        ...
        for device in self.list_subdir(data_dir):
            ...
            for subject in self.list_subdir(device_dir):
                ...
                if os.path.isdir(os.path.join(subject_dir, 'batterystats')):
                    row.update(self.aggregate_battery_final(
                        os.path.join(subject_dir, 'batterystats')))
                    ...
                    for browser in self.list_subdir(subject_dir):
                        ...
                        if os.path.isdir(os.path.join(browser_dir, 'batterystats')):
                            row.update(self.aggregate_battery_final(
                                os.path.join(browser_dir, 'batterystats')))
                            rows.append(row.copy())
        return rows
    ...
    def is_integer(self, number, minimum=0):
        ...
        if number < minimum:
            raise ConfigError(
                '%s should be equal or larger than %i' % (number, minimum))
        return number
...
class FileNotFoundError(Exception):
    ...
    def __init__(self, filename):
        Exception.__init__(self, '[Errno %s] %s: \'%s\'' % (

...
def parse_batterystats(app, batterystats_file, power_profile):
    ...
    with open(batterystats_file, 'r') as bs_file:
        ...
        phone_scanning_pattern = re.compile(
            r'(0|\+\d.*ms).*([+-])(phone_scanning)')
            ...
            if wifi_pattern.search(line):
                ...
                if wifi_activation == 0 and wifi_pattern.search(line).group(2) == '+' and current_time < app_end_time:
                    ...
                    if old_wifi_state == 'running':
                        wifi_intensity = get_amp_value(
                            power_profile, 'wifi.on')
                    if old_wifi_state == 'radio':
                        wifi_intensity = get_amp_value(
                            power_profile, 'wifi.active')
                    if old_wifi_state == 'scan':
                        wifi_intensity = get_amp_value(
                            power_profile, 'wifi.scan')
                    wifi_end_time = current_time
                    duration = wifi_end_time - wifi_start_time
                    if duration <= 0:
                        continue
                    energy_consumption = calculate_energy_usage(
                        wifi_intensity, voltage, duration)
        ...
        for component_pattern in component_patterns:
            ...
            for line in bs_file:
                if component_pattern.search(line):
                    ...
                    component_state_time = convert_to_s(
                        component_pattern.search(line).group(1))
                    component_intensity = get_amp_value(
                        power_profile, component)
                    if component_state == '+' and component_state_time < app_end_time:
                        ...
                        if (component_start_time < app_end_time) and (component_state_time > app_end_time):
                            ...
                            component_end_time = convert_to_s(
                                component_pattern.search(line).group(1))
                        duration = component_end_time - component_start_time
                        if duration != 0:
                            energy_consumption = calculate_energy_usage(
                                component_intensity, voltage, duration)
...
def get_screen_intensity(brightness, power_profile):
    ...
    intensity_range = get_amp_value(
        power_profile, 'screen.full') - get_amp_value(power_profile, 'screen.on')
    if brightness == 'dark':
        screen_intensity = get_amp_value(power_profile, 'screen.on')
    elif brightness == 'dim':
        screen_intensity = get_amp_value(
            power_profile, 'screen.on') + (intensity_range * 0.25)
    elif brightness == 'medium':
        screen_intensity = get_amp_value(
            power_profile, 'screen.on') + (intensity_range * 0.50)
    elif brightness == 'light':
        screen_intensity = get_amp_value(
            power_profile, 'screen.on') + (intensity_range * 0.75)
...
def get_timestamp():
    ts = t.time()
    st = dt.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S.%f')
    return st
...
def parse_systrace(app, systrace_file, logcat, batterystats, power_profile, core_amount):
    ...
    with open(systrace_file, 'r') as sys:
        f = sys.read()
        pattern = re.compile(
            '(?:<.{3,4}>-\d{1,4}|kworker.+-\d{3}).*\s(\d+\.\d+): (cpu_.*): state=(.*) cpu_id=(\d)')
        unix_time_pattern = re.compile(
            '(\d+\.\d+):\stracing_mark_write:\strace_event_clock_sync:\srealtime_ts=(\d+)')
        logcat_time = parse_logcat(app, logcat)
        systrace_time = float(unix_time_pattern.search(f).group(2))
        start_time = (logcat_time[0] - systrace_time) / \
            1000 + float(unix_time_pattern.search(f).group(1))
        end_time = (logcat_time[1] - systrace_time) / \
            1000 + float(unix_time_pattern.search(f).group(1))
        cpu_id_list = []
        results = []
        start_timestamp = (logcat_time[0] / 1000) + 7200
        end_timestamp = (logcat_time[1] / 1000) + 7200
            ...
            for match in matches:
                ...
                if current_time < start_time:
                    ...
                    if (found_first_match == 0) and (current_cpu_id == cpu_id) and current_time > start_time:
                        ...
                        activity = 'cpu_idle'
                        cpu_intensity = get_amp_value(
                            power_profile, 'cpu.idle')
                        energy_consumption = calculate_energy_usage(
                            cpu_intensity, voltage, duration)
                        # st = get_timestamp()
                        results.append('{},{},{},core {} {} start,{},{},{}'.format
                                       (time - start_time, current_time - start_time,
                                        duration, cpu_id, activity, energy_consumption, start_timestamp + (time - start_time), start_timestamp + (time - start_time) + duration))
                        time = current_time
...
def parse_logcat(app, logcat_file):
    ...
    with open(logcat_file, 'r') as f:
        ...
        time_tuple = t.strptime(
            '{}-{}'.format(year, app_start_date), '%Y-%m-%d %H:%M:%S')
        unix_start_time = int(t.mktime(time_tuple)) * 1000 + \
            int(app_start_pattern.search(logcat).group(2))
        ...
        time_tuple = t.strptime(
            '{}-{}'.format(year, app_stop_date), '%Y-%m-%d %H:%M:%S')
        unix_end_time = int(t.mktime(time_tuple)) * 1000 + \
            int(app_stop_pattern.search(logcat).group(2))
